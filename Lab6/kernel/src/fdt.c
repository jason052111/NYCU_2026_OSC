...
int get_parent_address_cells(const void* dtb_addr, const char *node_path, const char *cell_name) {
    ...
    if (last_slash == 0) parent_path[1] = '\0'; // Parent is the root node
    else if (last_slash > 0) parent_path[last_slash] = '\0'; // Truncate at the last slash
    ...
    if (parent_node >= 0) {
        ...
        if (prop != 0 && len == 4) return (int)bswap32(prop[0]); // Return the Big-Endian parsed value
    }
...
/*
 * Get a 32-bit property value from a DTB node.
 * The value is converted from big-endian to native endian.
 * Returns the property value on success, or -1 if the node is not found.
 */
unsigned long get_dtb_prop_u32(const void* dtb_addr, const char* path, const char* prop_name) {
    const void* fdt = dtb_addr;
...
void reserve_dtb(const void* fdt_ptr) {
    ...
    unsigned long start = kva_or_pa_to_pa((unsigned long)fdt_ptr);
