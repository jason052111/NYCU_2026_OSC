...
unsigned long INITRD_BASE_PA = 0;
unsigned long INITRD_BASE = 0xFFFFFFFF;

void set_initrd_address(unsigned long dtb_addr){
    INITRD_BASE_PA = initrd_address(dtb_addr);
    INITRD_BASE = PA2VA(INITRD_BASE_PA);
}
/*
...
void* load_user_program(const char* filename, unsigned long* size) {
    ...
    while (1) {
        ...
        if (strcmp(current_filename, filename) == 0) {
            void* prog_addr = allocate(PAGE_ALIGN_UP(filesize));
            ...
            mem_zero(prog_addr, PAGE_ALIGN_UP(filesize));
