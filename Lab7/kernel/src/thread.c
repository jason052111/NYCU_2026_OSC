...
static int init_task_stdio(struct task_struct* task);
/*
...
/*
 * Release all physical user pages mapped by a task.
 * Each mapped user page loses one reference. The physical page is
 * released when its reference count reaches zero.
 */
static void release_user_pages(struct task_struct* task) {
    if (task == 0 || task->pgd == 0) return;
    ...
    while (vma != 0) {
        for (unsigned long va = vma->start; va < vma->end; va += PAGE_SIZE) {
             ...
             */
            if (page_ref_get(pa) == 0) free((void*)PA2VA(pa));
...
/*
 * Release every VMA descriptor owned by a task.
 * This releases only VMA metadata. The physical pages should already
 * have been released by release_user_pages().
 */
...
/*
 * Release the page-table pages belonging to a user address space.
 * User leaf pages must already be released and unmapped. Kernel
 * high-half PGD entries are shared and must not be released here.
 */
static void free_user_pagetable(unsigned long* pgd) {
    ...
    for (int pgd_idx = 0; pgd_idx < PGD_INDEX(PAGE_OFFSET); pgd_idx++) {
        if (!(pgd[pgd_idx] & PTE_V)) continue;
        /*
         * user pgd entries point to PMD tables.
         */
        unsigned long* pmd = (unsigned long*)PA2VA(PTE2PA(pgd[pgd_idx]));
        for (int pmd_idx = 0; pmd_idx < ENTRIES_PER_TABLE; pmd_idx++) {
            if (!(pmd[pmd_idx] & PTE_V)) continue;
            /*
             * user pmd entries point to PTE tables.
             */
...
/*
 * Convert mmap protection flags into RISC-V PTE permission flags.
 */
...
/*
 * Check whether two address ranges overlap.
 * Returns nonzero when the ranges overlap, or zero otherwise.
 */
static int range_overlap(unsigned long a_start, unsigned long a_size, unsigned long b_start, unsigned long b_size) {
    unsigned long a_end = a_start + a_size;
    unsigned long b_end = b_start + b_size;
    return a_start < b_end && b_start < a_end;
}
/*
 * Check whether an address range is available for a new mmap region.
 */
static int mmap_addr_is_available(struct task_struct* task, unsigned long va, unsigned long size) {
    /*
     * Do not overlap user code region.
     * User code is mapped at [USER_CODE_VA, USER_CODE_VA + user_code_size).
     */
    if (range_overlap(va, size, USER_CODE_VA, task->user_code_size)) return 0;
    /*
     * Do not overlap user stack.
     */
    if (range_overlap(va, size, USER_STACK_BASE, USER_STACK_SIZE)) return 0;
    /*
     * Do not overlap previously allocated mmap region.
     * Simple bump allocator version:
     * mmap region is [USER_MMAP_BASE, task->mmap_base).
     */
    if (range_overlap(va, size, USER_MMAP_BASE, task->mmap_base - USER_MMAP_BASE)) return 0;
    return 1;
}
/*
 * Create a VMA descriptor and add it to the task's VMA linked list.
 * Returns the new VMA, or null if allocation fails.
 */
...
/*
 * Copy all VMA descriptors from a parent process to its child.
 * Only the VMA metadata is copied here. Physical pages are shared
 * separately using copy-on-write.
 */
static int copy_vma_list(struct task_struct* child, struct task_struct* parent) {
    child->vma_list = 0;
...
/*
 * Share one mapped user page between a parent and child using COW.
 */
static void cow_share_page(struct task_struct* parent, struct task_struct* child, unsigned long va) {
    unsigned long* parent_pte = lookup_pte(parent->pgd, va);
    if (parent_pte == 0 || !(*parent_pte & PTE_V)) return;
    unsigned long pa = PTE2PA(*parent_pte);
    unsigned long flags = PTE_FLAGS(*parent_pte);
    /*
     * Only handle user pages.
     */
    if (!(flags & PTE_U)) return;
    /*
...
/*
 * Share all currently mapped user pages between parent and child.
 * Pages that have not been demand-paged yet are skipped. Each process
 * can allocate those pages independently when it faults later.
 */
static void cow_share_vmas(struct task_struct* parent, struct task_struct* child) {
...
/*
 * Initialize the VFS state of a newly created task.
 */
...
/*
 * Close every file descriptor owned by a task.
 */
static void close_task_files(struct task_struct* task) {
    if (task == 0) return;
    for (int i = 0; i < MAX_FD; i++) {
...
/*
 * Open /dev/uart as stdin, stdout, and stderr for a user task.
 * Returns 0 on success, or -1 if one of the opens fails.
 */
static int init_task_stdio(struct task_struct* task) {
    if (task == 0) return -1;
...
/*
 * Find the VMA containing a given virtual address.
 * Returns the matching VMA, or null if the address does not belong
 * to any valid virtual-memory region owned by the task.
 */
...
/*
 * Create an anonymous virtual-memory mapping for the current process.
 *
 * Without MAP_POPULATE, only a VMA is created and physical pages are
 * allocated later through demand paging. Returns the starting virtual
 * address on success, or (void*)-1 on failure.
 */
void* process_mmap(void* addr, unsigned long length, int prot, int flags) {
    struct task_struct* current = get_current();

    if (current == 0 || current->type != TASK_USER_PROCESS) return (void*)-1;
    ...
    if (addr != 0 && ((hint & (PAGE_SIZE - 1)) == 0) && mmap_addr_is_available(current, hint, size)) {
        va = hint;
        if (va + size > current->mmap_base) current->mmap_base = va + size;
    } else {
