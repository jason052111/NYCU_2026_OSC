...
void* allocate(unsigned long size) {
    ...
    if (target_pool == NULL) {
        ...
        return (void*)PA2VA(page_2_physical(p));
    }
    /*
    ...
    if (target_pool->free_list == NULL) {
        ...
        unsigned long page_addr = PA2VA(page_2_physical(p));
...
void free(void* ptr) {
    ...
    unsigned long base_page_addr = kva_or_pa_to_pa((unsigned long)ptr) & ~(PAGE_SIZE - 1);
...
void all_mem_addr_size(unsigned long dtb_addr){
    unsigned long kernel_base = kva_or_pa_to_pa((unsigned long)_start);
    unsigned long kernel_size = kva_or_pa_to_pa((unsigned long)_end) - kernel_base ;
...
void allocate_mem_map() {
    ...
    mem_map = (struct page*)PA2VA(try_start);
