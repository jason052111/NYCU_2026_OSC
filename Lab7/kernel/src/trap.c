...
/*
 * Check whether the VMA permits the operation that caused the page fault.
 */
static unsigned long fault_type_allowed(struct vm_area* vma, unsigned long cause) {
    if (cause == 12) return (vma->prot & PTE_X);
    if (cause == 13) return (vma->prot & PTE_R);
    if (cause == 15) return (vma->prot & PTE_W);
    return 0;
}
/*
 * Handle a write fault on a copy-on-write page.
 * Returns:
 *   1  if the COW fault was handled successfully.
 *   0  if this is not a COW fault.
 *  -1  if the COW fault cannot be handled.
 */
static int handle_cow_fault(struct task_struct* current, struct vm_area* vma, unsigned long addr) {
    unsigned long page_va = PAGE_ALIGN_DOWN(addr);
    unsigned long* pte = lookup_pte(current->pgd, page_va);

    if (pte == 0 || !(*pte & PTE_V)) return 0;
    unsigned long flags = PTE_FLAGS(*pte);
    if (!(flags & PTE_COW)) return 0;
    // If the VMA is not writable, this is not a valid COW fault.
    if (!(vma->prot & PTE_W)) return -1;
    unsigned long old_pa = PTE2PA(*pte);

    if (page_ref_get(old_pa) <= 1) {
        /*
         * Only this process owns it now.
         * Just restore write permission.
         */
        flags |= PTE_W;
        flags &= ~PTE_COW;
    ...
    mem_copy(new_page, (void*)PA2VA(old_pa), PAGE_SIZE);
    unsigned long new_pa = VA2PA((unsigned long)new_page);
...
/*
 * Handle instruction, load, and store page faults.
 * This function handles demand paging, file-backed pages, and COW.
 */
static void handle_page_fault(struct trap_frame* regs)
{
    struct task_struct* current = get_current();
    unsigned long addr = regs->stval;
    unsigned long page_va = PAGE_ALIGN_DOWN(addr);
    unsigned long cause = regs->scause & 0xff;
    // A page fault without a valid user process is a kernel error.
    if (current == 0 || current->type != TASK_USER_PROCESS) {
        uart_puts("=== Kernel page fault ===\n");
        while (1) { }
    }
    // Find the VMA containing the faulting virtual address.
    struct vm_area* vma = find_vma(current, addr);
    // The address is invalid if it is outside every VMA.
    if (vma == 0) {
        // uart_puts("[Segmentation fault]: Kill Process\n");
        // uart_puts("Invaid address\n");
        process_exit(-1);
        return;
    }
    // A store page fault may have been caused by copy-on-write.
    if (cause == 15) {
        int cow_ret = handle_cow_fault(current, vma, addr);
        // The COW fault was handled successfully.
        if (cow_ret == 1) return;
        // The COW fault was detected but could not be handled.
        if (cow_ret < 0) {
            // uart_puts("[Segmentation fault]: Kill Process\n");
            process_exit(-1);
            return;
    ...
    // Look for an existing PTE associated with the faulting page.
    unsigned long* old_pte = lookup_pte(current->pgd, page_va);
    /*
     * A valid mapping already exists, but the fault was not handled
     * as COW. Therefore, this is a real permission violation.
     */
    if (old_pte != 0 && (*old_pte & PTE_V)) {
        /*
        * PTE already exists, but it was not handled as COW.
        * This is a real permission fault.
        */
        // uart_puts("[Segmentation fault]: Kill Process\n");
        process_exit(-1);
        return;
    }
    // Check whether the VMA permits the faulting access operation.
    if (!fault_type_allowed(vma, cause)) {
        // uart_puts("[Segmentation fault]: Kill Process\n");
        process_exit(-1);
        return;
    }
    ...
    // Terminate the process if page allocation fails.
    if (page == 0) {
        // uart_puts("[Segmentation fault]: Kill Process\n");
        process_exit(-1);
        return;
    ...
    if (vma->type == VMA_FILE) {
        ...
        if (offset < vma->file_size) remain = vma->file_size - offset;
        unsigned long copy_size = remain > PAGE_SIZE ? PAGE_SIZE : remain;
        if (copy_size > 0) mem_copy(page, (void*)((unsigned long)vma->file_data + offset), copy_size);
    }

    map_pages(current->pgd, page_va, VA2PA((unsigned long)page), PAGE_SIZE, vma->prot);
    switch_vm(current->pgd);
