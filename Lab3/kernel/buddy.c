...
struct page* alloc_pages(unsigned int order) {
    ...
    uart_hex(page_2_physical(target_page));
    uart_puts(" at order ");
    uart_int(order);
    uart_puts(", page ");
    uart_int(target_page - mem_map);
    uart_puts(". Next address at order ");
    uart_int(order);
    uart_puts(": ");
    if (free_area[order] != NULL) {
        uart_hex(page_2_physical(free_area[order]));
    } else {
        uart_puts("None");
    }
    uart_puts("\n");
...
void free_pages(struct page* p) {
    p->refcount = 0; 
    p->chunk_size = 0;
    unsigned long orig_addr = page_2_physical(p);
    ...
    uart_puts(": ");
    if (p->next != NULL) {
        uart_hex(page_2_physical(p->next));
    } else {
        uart_puts("None");
    }
    uart_puts("\n");
...
void* allocate(unsigned long size) {
    ...
    uart_hex((unsigned long)allocated_chunk);
    uart_puts(" at chunk size ");
    uart_int(target_pool->chunk_size);
    uart_puts("\n");
...
void allocate_mem_map() {
    ...
    uart_puts("[Info] mem_map placed at ");
    uart_hex(try_start);
    uart_puts(", size: ");
    uart_int(map_size);
    uart_puts(" bytes\n");
